...
const DEFAULT={goal:1800,total:0,history:[],plantLevel:0,lastDate:null,reminders:[{time:'10:30',text:'🌱 Псс… Я немного хочу пить 💧',enabled:true},{time:'13:00',text:'🥺 Кажется, моя земля становится сухой...',enabled:true},{time:'16:00',text:'💦 250 мл — и я снова буду счастливым!',enabled:true},{time:'19:00',text:'🌿 Ты сегодня хорошо обо мне заботилась ❤️',enabled:true}]};
let state=load();
...
$('#settingsBtn').onclick=()=>{$('#goalInput').value=state.goal;$('#settingsModal').classList.remove('hidden')};$('#saveGoal').onclick=()=>{state.goal=Math.max(500,Number($('#goalInput').value)||1800);save();render();$('#settingsModal').classList.add('hidden')};
